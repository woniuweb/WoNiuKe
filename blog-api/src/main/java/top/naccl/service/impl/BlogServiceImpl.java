...
public class BlogServiceImpl implements BlogService {
	...
	public Blog getBlogById(Long id) {
		...
		// 将浏览量设置为Redis中的最新值
		Object viewObj = redisService.getValueByHashKey(RedisKeyConstants.BLOG_VIEWS_MAP, blog.getId());
		int view;
		if (viewObj == null) {
			// Redis 中没有该博客的浏览量，使用数据库中的值
			view = blog.getViews();
			// 更新 Redis 中的数据
			redisService.saveKVToHash(RedisKeyConstants.BLOG_VIEWS_MAP, blog.getId(), view);
		} else {
			view = (int) viewObj;
		}
		blog.setViews(view);
